...
# Configure PowerShell Profile with Auto-Update
function Configure-Profile {
    Write-Host "⚙️ Configuring PowerShell profile with auto-update..." -ForegroundColor Yellow
    ...
    # Create custom Oh My Posh config with auto-upgrade enabled
    $configPath = "$profileDir\ohmyposh-config.json"

    # Download the selected theme and modify it to add auto-upgrade
    $themePath = "$env:POSH_THEMES_PATH\$Theme.omp.json"
    if (Test-Path $themePath) {
        $themeContent = Get-Content $themePath -Raw | ConvertFrom-Json
    }
    else {
        # Fallback to a basic configuration if theme not found
        $themeContent = @{
            version = 2
            blocks = @(
                @{
    ...
    # Add auto-upgrade configuration
    $themeContent | Add-Member -MemberType NoteProperty -Name "auto_upgrade" -Value $true -Force

    # Add upgrade segment to the first block if it doesn't exist
    if ($themeContent.blocks -and $themeContent.blocks[0].segments) {
        $upgradeSegment = @{
            type = "upgrade"
            style = "plain"
            foreground = "#FFD700"
        ...
        # Insert upgrade segment at the beginning
        $themeContent.blocks[0].segments = @($upgradeSegment) + $themeContent.blocks[0].segments
    }

    # Save the modified configuration
    $themeContent | ConvertTo-Json -Depth 10 | Set-Content -Path $configPath -Encoding UTF8
...
# Oh My Posh initialization with auto-upgrade enabled
...
# Welcome message
Write-Host "🚀 PowerShell environment loaded with Oh My Posh (auto-update enabled) and fzf!" -ForegroundColor Green
Write-Host "💡 Use 'Update-OhMyPosh' to manually check for updates" -ForegroundColor Cyan
"@

    # Write profile content
    Set-Content -Path $profilePath -Value $profileContent -Encoding UTF8

    Write-Host "✅ PowerShell profile configured with auto-update enabled" -ForegroundColor Green
...
try {
    ...
    # Configure profile with auto-update
    Configure-Profile
    ...
    Write-Host "   - Oh My Posh will check for updates automatically" -ForegroundColor White
    Write-Host "   - Update notifications will appear in your prompt" -ForegroundColor White
    Write-Host "   - Updates will be installed automatically when available" -ForegroundColor White
